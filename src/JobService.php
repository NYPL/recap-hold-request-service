...
class JobService
{
    ...
    /**
     * @param RecapCancelHoldRequest $recapCancelHoldRequest
     * @param string $message
     */
    public static function beginJob(RecapCancelHoldRequest $recapCancelHoldRequest, $message = '')
    {
        self::initializeJobClient();
        self::buildJobNotice($recapCancelHoldRequest->getRawData(), $message);
        self::getJobStatus()->setNotice(self::getJobNotice());

        APILogger::addDebug(
            'Job is being initiated via the Job Service API. (ID: ' . $recapCancelHoldRequest->getId() . ')',
            [self::getJobStatus()]
        );
